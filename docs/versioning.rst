...
* Version identifiers are strings that adhere to semantic versioning, and the
  ...
  addition made across all software products
* Release commits will be tagged using ``vX.Y.Z`` in accordance with
  ``setuptools_scm`` requirements to allow for automatic versioning of the
  Python package
...
version of the Python package is managed automatically by the use the
``setuptools_scm`` in ``pyproject.toml``.  The version identifier for the source
