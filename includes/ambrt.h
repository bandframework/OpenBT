...
public:
   ...
   void setstats(bool dostats) { mi.dostats=dostats; for(size_t j=0;j<m;j++) mb[j].setstats(dostats); if(dostats) { delete[] mi.varcount; mi.varcount=new unsigned int[xi->size()](); } }
   void pr();
