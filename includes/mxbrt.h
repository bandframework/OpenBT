...
public:
    ...
    virtual std::vector<sinfo*>& newsinfovec(size_t dim) { std::vector<sinfo*>* si = new std::vector<sinfo*>; si->reserve(dim); for(size_t i=0;i<dim;i++) si->push_back(new mxsinfo(k)); return *si; }
